...
def epsilon_greedy(state_vector, epsilon):
    ...
    if take_rand_action:
        ...
        with torch.no_grad():
            ...
            action_index = np.argmax(q_values_action)
            object_index = np.argmax(q_values_object)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_action_vec = q_value_cur_state[0]
    q_object_vec = q_value_cur_state[1]
    
    q_val_cur_state_action = 1/2 * (q_action_vec[action_index] + q_object_vec[object_index])

    # TODO Your code here
    # calc target ( reward + gamma * max Q next step)
    # calc estimate (input state vector applied on current model)
    # calc loss from target vs estimate
    # then let model update

    if type(reward) == int:
        reward = float(reward)
    targety = reward + GAMMA * maxq_next
    if terminal:
        targety = torch.tensor(reward)
        

    # loss = None
    lossfn = nn.MSELoss()
    loss = lossfn(q_val_cur_state_action, targety)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
