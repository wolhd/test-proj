...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    """
    # bic = loglik - 1/2 numFreeParam log(numData)
    # bic = loglik - 1/2 * numParams * log(N)
    
    numParams = 0
    numParams += mixture.mu.size
    numParams += mixture.var.size
    numParams += mixture.p.size -1
    N = X.shape[0]
    bic = log_likelihood - 0.5 * numParams * np.log(N)
    return bic
